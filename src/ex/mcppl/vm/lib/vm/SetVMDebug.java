...
public class SetVMDebug implements BasicLibrary {
    ...
    public ExObject invoke(HashMap<String, ExObject> values, Executor executor) throws VMRuntimeException {
        ...
        if (data == null) throw new VMRuntimeException("vm.debug : Not found value 'status'");
        if (data instanceof ExValueName) {
            ...
            if(v==null)throw new VMRuntimeException("vm.debug : Not found value.");
            if(!(v.getValue() instanceof ExBool))throw new VMRuntimeException("The variable '"+v.getName()+"' cannot be converted to a boolean type.");
            data = new ExBool(Boolean.parseBoolean(v.getValue().getData()));
        }
        if(!(data instanceof ExBool))throw new VMRuntimeException("The value '"+data.getData()+"' cannot be converted to a boolean type.");
