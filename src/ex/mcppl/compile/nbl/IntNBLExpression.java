...
public class IntNBLExpression {
    ...
    public ArrayList<ByteCode> calculate(ArrayList<LexToken.TokenD> tds)throws VMException {
        ...
        for(LexToken.TokenD td:tds){
            ...
            if(td.getToken().equals(LexToken.Token.NUM))bbc.add(new PushByteCode(new ExInt(Integer.parseInt(td.getData()))));
            else if(td.getToken().equals(LexToken.Token.KEY)){
                ExValueName valuea = null;
                if(Element.value_names.contains(td.getData())){
                    valuea = new ExValueName(td.getData());
                }else throw new VMException("Unknown value name.");
